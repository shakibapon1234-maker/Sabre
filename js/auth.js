...
(() => {
  ...
  function lock(message) { clearInterval(heartbeat); session = null; localStorage.removeItem(SESSION_KEY); sessionStorage.removeItem(SESSION_KEY); $('sbLoginError').textContent = message || 'Internet connection is required to use this training application.'; $("sbLoginShell").style.display = ""; }
  ...
  async function signIn(event) { event.preventDefault(); const username = $('sbLoginUsername').value.trim().toUpperCase(), password = $('sbLoginPassword').value; $('sbLoginError').textContent = ''; try { const login = await call('login-with-device', { username, password, device_id:deviceId, device_name:navigator.userAgent.slice(0,150), app_type:appType() }); await call('device-license', { action:'activate', device_id:deviceId, device_name:navigator.userAgent.slice(0,150), app_type:appType() }, login.session.access_token); session = { ...login.session, profile:login.profile }; sessionStorage.setItem(SESSION_KEY, JSON.stringify(session)); $("sbLoginShell").style.display = "none"; startHeartbeat(); } catch (e) { $('sbLoginError').textContent = e.message; } }
